...
namespace BMLRealizer
{
    ...
    public class BMLFaceFacs : BMLFace
    {
        ...
        {
            base.Parse(reader);
            au = TryParseAtribute<string>(reader, "au", "1", true);

            string strSide = TryParseAtribute<string>(reader, "side", "BOTH", false);
            if(strSide.ToLower() == "left") {
                side = Side.LEFT;
            }
            else if(strSide.ToLower() == "right") {
                side = Side.RIGHT;
            }
        }
